...
@section('page-title', $sale->bill_type === 'non_gst' ? 'Estimate & Retail Bill' : 'Tax Invoice & Receipt')
...
@php
    ...
    $waMsg .= "🏬 *{$storeName}*\n";
    $waMsg .= "------------------------------------\n";
    $waMsg .= "Hello *{$sale->customer_name}*,\n";
    $waMsg .= "Thank you for purchasing with us! Here is your bill summary:\n\n";
    $waMsg .= "📌 *Invoice #:* {$sale->invoice_number}\n";
    $waMsg .= "📅 *Date:* " . date('d M Y, h:i A', strtotime($sale->created_at)) . "\n";
    $waMsg .= "📱 *Device:* {$sale->brand} {$sale->model} (" . ($sale->ram ? "{$sale->ram}/{$sale->storage}" : "Standard") . ")\n";
    $waMsg .= "🔢 *IMEI 1:* {$sale->imei_1}\n";
    ...
    $waMsg .= "🛡️ *Warranty:* 1 Year Official Brand Warranty.\n";
    $waMsg .= "Thank you for visiting us! Please save this message for your records.\n";
...
@section('page-actions')
    <div style="display:flex; gap: 8px; align-items:center; flex-wrap:wrap;">
        ...
        <a href="{{ route('mobileshop.invoice.pdf', ['id' => $sale->id]) }}" class="btn btn-primary btn-sm" style="font-weight:700; background:#5E6AD2;">
...
<div class="invoice-page-wrapper" style="width: 100%; margin: 0; padding-bottom: 40px; display:flex; justify-content:center;">
    ...
    <!-- ─── DESIGNED A4 GST TAX INVOICE (LUXURY LINEAR STYLE) ─── -->
    <div id="viewA4" class="printable-invoice-container" style="width: 100%; max-width: 860px; background: #ffffff; border: 1px solid #E2E8F0; border-radius: 12px; padding: 0; color: #0F172A; box-shadow: 0 4px 20px rgba(0,0,0,0.06); overflow: hidden; position: relative;">
        ...
        <div style="padding: 32px 36px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px;">
                        ...
                        <div>
                            ...
                            <div style="font-size: 11px; font-weight: 600; color: #5E6AD2; letter-spacing: 0.5px; text-transform: uppercase; margin-top: 2px;">
                                Flagship Retail &amp; Authorized Smartphone Hub
                            </div>
                        </div>
                    </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-top: 20px; margin-bottom: 24px;">
                ...
                <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 10px; padding: 14px 16px;">
                    ...
                    <div style="font-size: 12px; color: #334155; margin-bottom: 2px;">
                        Phone: <strong style="color: #0F172A;">{{ $sale->customer_phone ?: 'Walk-in Customer' }}</strong>
                    </div>
                    ...
                    <div style="font-size: 11px; color: #64748B; margin-top: 4px;">
                        {{ $sale->customer_address ?: 'Store Counter Delivery · Mumbai Metro' }}
                    </div>
                ...
                <!-- Card 2: Payment & Fulfillment Details -->
                    ...
                    <div>
                        ...
                        <div style="font-size: 12px; color: #334155; line-height: 1.6;">
                            ...
                            @if($sale->payment_mode === 'emi' && $emiProvider)
                                <div style="margin-top: 2px; color: #4338CA; font-weight: 600;">
                                    Financier: <strong>{{ $emiProvider->name }}</strong>
                                    @if($sale->emi_loan_no) &bull; Loan No: <span style="font-family:'JetBrains Mono', monospace;">{{ $sale->emi_loan_no }}</span>@endif
                                </div>
                                @if($sale->emi_downpayment > 0)
                                    <div style="font-size: 11px; color: #64748B;">
                                        Downpayment: ₹{{ number_format($sale->emi_downpayment, 2) }} &bull; Financed: ₹{{ number_format($sale->emi_financed_amount, 2) }}
                                    </div>
                                @endif
                            @endif
                            ...
                            <div style="margin-top: 2px;">
                                @if($sale->bill_type === 'gst')
                                    Tax Regime: <strong style="color: #0F172A;">{{ strtoupper(str_replace('_', ' ', $sale->tax_type)) }} @ 18% GST</strong>
                                @else
                                    Category: <strong style="color: #0F172A;">Retail Sale / Non-GST Estimate</strong>
                                @endif
                            </div>

                            <div style="margin-top: 2px; font-size: 11px; color: #64748B;">
                                Warranty Coverage: <strong style="color: #047857;">1 Year Official Manufacturer Warranty</strong>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- ════ 3. LINE ITEMS TABLE (PREMIUM STYLED) ════ -->
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 11.5px;">
                    <thead>
                        <tr style="background: #0F172A; color: #FFFFFF;">
                            ...
                            <th style="padding: 10px 14px; text-align: left; font-weight: 700; font-size: 9.5px; text-transform: uppercase; letter-spacing: 0.6px; width: 44%;">Item Description &amp; Hardware Details</th>
                            ...
                            <th style="padding: 10px 12px; text-align: right; font-weight: 700; font-size: 9.5px; text-transform: uppercase; letter-spacing: 0.6px; width: 12%;">Rate (₹)</th>
                    ...
                    <tbody>
                        <!-- Primary Mobile Device Row -->
                        <tr style="border-bottom: 1px solid #E2E8F0; background: #FFFFFF;">
                            <td style="padding: 14px 12px; text-align: center; font-weight: 700; color: #64748B;">1</td>
                            <td style="padding: 14px;">
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <span style="font-size: 13.5px; font-weight: 800; color: #0F172A; letter-spacing: -0.2px;">
                                        {{ $sale->brand }} {{ $sale->model }}
                                    </span>
                                    <span style="background: #EEF2FF; color: #4338CA; border: 1px solid #C7D2FE; font-size: 9px; font-weight: 700; padding: 1px 6px; border-radius: 4px; text-transform: uppercase;">
                                        Brand New
                                    </span>
                                </div>
                                ...
                                <div style="display: flex; flex-wrap: wrap; gap: 6px; margin-top: 4px;">
                                    @if($sale->ram || $sale->storage)
                                        <span style="background: #F1F5F9; color: #334155; font-size: 10px; font-weight: 600; padding: 1px 6px; border-radius: 4px; border: 1px solid #E2E8F0;">
                                            {{ $sale->ram ? $sale->ram . ' RAM / ' . $sale->storage . ' ROM' : $sale->storage }}
                                        </span>
                                    @endif
                                    @if($sale->color)
                                        <span style="background: #F1F5F9; color: #334155; font-size: 10px; font-weight: 600; padding: 1px 6px; border-radius: 4px; border: 1px solid #E2E8F0;">
                                    ...
                                    @endif
                                    <span style="background: #ECFDF5; color: #065F46; font-size: 10px; font-weight: 600; padding: 1px 6px; border-radius: 4px; border: 1px solid #A7F3D0;">
                                        ✓ 1 Year Brand Warranty
                                    </span>
                                </div>

                                <!-- Hardware Identification Box (IMEI) -->
                                <div style="margin-top: 8px; background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 6px; padding: 6px 10px; display: inline-block;">
                                    ...
                                    <div style="font-family: 'JetBrains Mono', monospace; font-size: 10.5px; font-weight: 700; color: #0F172A; margin-top: 2px;">
                                        IMEI 1: {{ $sale->imei_1 }}
                                        @if($sale->imei_2)
                                            &nbsp;&bull;&nbsp; IMEI 2: {{ $sale->imei_2 }}
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td style="padding: 14px 10px; text-align: center; font-family: 'JetBrains Mono', monospace; font-size: 11px; color: #334155;">
                                {{ $sale->hsn_code ?: '85171300' }}
                            </td>
                            <td style="padding: 14px 8px; text-align: center; font-weight: 800; color: #0F172A; font-size: 12px;">
                                1
                            </td>
                            <td style="padding: 14px 12px; text-align: right; font-family: 'JetBrains Mono', monospace; font-size: 11.5px; color: #334155;">
                                {{ number_format($sale->sale_price / 1.18, 2) }}
                            </td>
                            <td style="padding: 14px 12px; text-align: right; font-family: 'JetBrains Mono', monospace; font-size: 11.5px; color: #334155;">
                                {{ number_format($sale->sale_price / 1.18, 2) }}
                            </td>
                            <td style="padding: 14px; text-align: right; font-family: 'JetBrains Mono', monospace; font-weight: 800; font-size: 13px; color: #0F172A;">
                        ...
                        @if(isset($gifts) && $gifts->count() > 0)
                            ...
                            <tr style="border-bottom: 1px solid #E2E8F0; background: #F0FDF4;">
                                <td style="padding: 10px 12px; text-align: center; font-weight: 700; color: #166534;">{{ $idx + 2 }}</td>
                                <td style="padding: 10px 14px;">
                                    <div style="display: flex; align-items: center; gap: 6px;">
                                        <span style="font-weight: 700; color: #166534; font-size: 11.5px;">🎁 Free Gift Promo: {{ $g->name }}</span>
                                        <span style="background: #DCFCE7; color: #15803D; font-size: 8.5px; font-weight: 800; padding: 1px 5px; border-radius: 4px;">FREE</span>
                                    </div>
                                    <div style="font-size: 10px; color: #15803D; margin-top: 1px;">Store complimentary promotional accessory</div>
                                </td>
                                <td style="padding: 10px; text-align: center; font-family: 'JetBrains Mono', monospace; color: #166534; font-size: 10.5px;">85177090</td>
                                <td style="padding: 10px; text-align: center; font-weight: 700; color: #166534;">{{ $g->qty }}</td>
                                <td style="padding: 10px 12px; text-align: right; font-family: 'JetBrains Mono', monospace; color: #166534;">0.00</td>
                                <td style="padding: 10px 12px; text-align: right; font-family: 'JetBrains Mono', monospace; color: #166534;">0.00</td>
                                <td style="padding: 10px 14px; text-align: right; font-weight: 800; font-family: 'JetBrains Mono', monospace; color: #15803D;">FREE</td>
                            </tr>
                            @endforeach
                        @endif
                    </tbody>
                                ...
                                <tbody>
                                    @if($sale->tax_type === 'intra_state')
                                        <tr style="border-bottom: 1px solid #F1F5F9;">
                                            ...
                                            <td style="padding: 6px 10px; text-align: right; font-family: 'JetBrains Mono', monospace; color: #334155;">{{ number_format($sale->sale_price / 1.18, 2) }}</td>
                    ...
                    <div style="padding: 14px 16px; border-bottom: 1px solid #E2E8F0; background: #F8FAFC;">
                        <div style="display: flex; justify-content: space-between; font-size: 12px; color: #475569; margin-bottom: 6px;">
                            ...
                            <span style="font-family: 'JetBrains Mono', monospace; font-weight: 600; color: #0F172A;">
                                ₹{{ number_format($sale->bill_type === 'gst' ? ($sale->sale_price / 1.18) : $sale->total_amount, 2) }}
                            </span>
                        </div>
            ...
            <!-- ════ 5. STORE ASSURANCE & WARRANTY TRUST BADGES ════ -->
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-bottom: 22px; padding: 10px; background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; text-align: center;">
                <div style="font-size: 11px; font-weight: 700; color: #0F172A; display: flex; align-items: center; justify-content: center; gap: 5px;">
                    <span>🛡️</span> 100% Genuine Handset
                </div>
                <div style="font-size: 11px; font-weight: 700; color: #0F172A; display: flex; align-items: center; justify-content: center; gap: 5px; border-left: 1px solid #E2E8F0; border-right: 1px solid #E2E8F0;">
                    <span>⚡</span> 7-Day Replacement Support
                </div>
                <div style="font-size: 11px; font-weight: 700; color: #0F172A; display: flex; align-items: center; justify-content: center; gap: 5px;">
            ...
            <div style="display: grid; grid-template-columns: 1.2fr 0.8fr; gap: 24px; padding-top: 16px; border-top: 1px solid #E2E8F0; align-items: flex-end;">
                ...
                <div>
                    ...
                    <ol style="font-size: 9px; color: #64748B; padding-left: 13px; line-height: 1.5; margin: 0;">
                        <li>1-Year manufacturer warranty on brand new smartphones is honored directly at official brand service centers.</li>
                        <li>Physical drop damage, screen cracks, or liquid intrusion void the manufacturer warranty.</li>
                        <li>Dead-on-Arrival (DOA) replacement is covered within 7 days from purchase date subject to brand service verification.</li>
                        <li>Goods once sold cannot be returned for cash refund. All disputes are subject to local Mumbai jurisdiction.</li>
                    </ol>
                </div>
    ...
    <div id="viewThermal" class="card" style="display:none; width: 100%; max-width: 320px; margin: 0 auto; padding: 18px; font-family: 'Courier New', monospace; font-size: 11px; color: #111827; background:#fff; border:1px dashed #9CA3AF; border-radius: 6px;">
        ...
        <div style="border-top: 1px dashed #CBD5E1; padding-top: 6px; margin-top: 6px; font-size:10px;">
            <div style="display:flex; justify-content:space-between;"><span>Taxable:</span><span>₹{{ number_format($sale->sale_price / 1.18, 2) }}</span></div>
