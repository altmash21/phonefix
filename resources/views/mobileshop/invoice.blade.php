...
@php
    ...
    $waMsg .= "ðŸ“± *Device:* {$sale->brand} {$sale->model} (" . ($sale->ram ? "{$sale->ram}/{$sale->storage}" : "Standard") . ")\n";
    $waMsg .= "ðŸ”¢ *IMEI 1:* {$sale->imei_1}\n";
    if (!empty($sale->imei_2)) {
        $waMsg .= "ðŸ”¢ *IMEI 2:* {$sale->imei_2}\n";
    }
    ...
    $waMsg .= "ðŸ›¡ï¸ *Warranty:* 1 Year Brand Warranty.\n";
...
@section('page-actions')
    <div style="display:flex; gap: 8px; align-items:center; flex-wrap:wrap;">
        ...
        <a href="{{ route('mobileshop.invoice.pdf', ['id' => $sale->id]) }}" class="btn btn-primary btn-sm" style="font-weight:700; background:#0F766E;">
...
<div class="invoice-page-wrapper" style="width: 100%; margin: 0; padding-bottom: 40px;">
    ...
    <div id="viewThermal" class="card" style="display:none; max-width: 320px; margin: 0 auto; padding: 18px; font-family: 'Courier New', monospace; font-size: 11px; color: #111827; background:#fff; border:1px dashed #9CA3AF;">
        ...
        <div style="border-top: 1px dashed #CBD5E1; padding-top: 6px; margin-top: 6px; font-size:10px;">
            <div style="display:flex; justify-content:space-between;"><span>Taxable:</span><span>â‚¹{{ number_format($sale->sale_price / 1.18, 2) }}</span></div>
            <div style="display:flex; justify-content:space-between;"><span>GST (18%):</span><span>â‚¹{{ number_format($sale->cgst_amount + $sale->sgst_amount + $sale->igst_amount, 2) }}</span></div>
