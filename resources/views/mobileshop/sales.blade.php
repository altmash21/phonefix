                ...
                <tbody>
                    ...
                    <tr class="sales-row" data-type="{{ ($sale->device_type ?? '') === 'second_hand' ? 'secondhand' : 'phone' }}" data-date="{{ \Carbon\Carbon::parse($sale->created_at)->format('Y-m-d') }}">
                        ...
                        <td style="text-align:center; white-space:nowrap;">
                            ...
                            <div style="display:inline-flex; gap:6px; align-items:center;">
                                <a href="{{ route('mobileshop.invoice', ['id' => $sale->id]) }}" class="btn btn-outline btn-sm" style="padding:4px 8px; font-size:11px; display:inline-flex; align-items:center; gap:4px;">
                                    <i data-lucide="printer" style="width:12px;height:12px;"></i> View Bill
                                </a>
                                <a href="{{ $mobWaUrl }}" target="_blank" class="btn btn-outline btn-sm" style="padding:4px 8px; font-size:11px; color:#16A34A; border-color:#BBF7D0; background:#F0FDF4; display:inline-flex; align-items:center; gap:4px;" title="Share Invoice on WhatsApp">
                                ...
                                @if(($sale->status ?? '') !== 'voided')
                                    <button type="button" onclick="handleReturnButtonClick(this, 'mobile', {{ $sale->id }}, '{{ $sale->invoice_number }}', '{{ number_format($sale->total_amount, 2) }}')" data-items="{{ json_encode($sale->items ?? []) }}" class="btn btn-outline btn-sm" style="padding:4px 8px; font-size:11px; color:#DC2626; border-color:#FECDD3; background:#FFF1F2; display:inline-flex; align-items:center; gap:4px; cursor:pointer;" title="Process Return & Restock">
                    ...
                    <tr class="sales-row" data-type="accessory" data-date="{{ \Carbon\Carbon::parse($asale->created_at)->format('Y-m-d') }}">
                        ...
                        <td style="text-align:center; white-space:nowrap;">
                            ...
                            @endphp
                            <div style="display:inline-flex; gap:6px; align-items:center;">
                                <a href="{{ route('mobileshop.accessories.invoice', ['id' => $asale->id]) }}" class="btn btn-outline btn-sm" style="padding:4px 8px; font-size:11px; display:inline-flex; align-items:center; gap:4px;">
                                    <i data-lucide="printer" style="width:12px;height:12px;"></i> View Bill
                                </a>
                                <a href="{{ $accWaUrl }}" target="_blank" class="btn btn-outline btn-sm" style="padding:4px 8px; font-size:11px; color:#16A34A; border-color:#BBF7D0; background:#F0FDF4; display:inline-flex; align-items:center; gap:4px;" title="Share Invoice on WhatsApp">
                                    <svg style="width:12px;height:12px;fill:currentColor;" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
                                    WhatsApp
                                </a>
                                @if(($asale->status ?? '') !== 'voided')
                                    <button type="button" onclick="handleReturnButtonClick(this, 'accessory', {{ $asale->id }}, '{{ $asale->invoice_number }}', '{{ number_format($asale->total_amount, 2) }}')" data-items="{{ json_encode($asale->items ?? []) }}" class="btn btn-outline btn-sm" style="padding:4px 8px; font-size:11px; color:#DC2626; border-color:#FECDD3; background:#FFF1F2; display:inline-flex; align-items:center; gap:4px; cursor:pointer;" title="Process Return & Restock">
                                        <i data-lucide="rotate-ccw" style="width:12px;height:12px;"></i> Return
                                    </button>
                                @else
                                    <span class="badge" style="background:#FEE2E2; color:#DC2626; font-size:10px; font-weight:700;">Returned</span>
    ...
    function renderCart() {
        ...
        cart.forEach((item, index) => {
            ...
            <tr>
                ...
                <td style="text-align:center;">
                    <button type="button" style="background:none; border:none; color:#DC2626; font-size:16px; cursor:pointer;" onclick="removeCartItem(${index})">✕</button>
                </td>
            </tr>`;
        });
