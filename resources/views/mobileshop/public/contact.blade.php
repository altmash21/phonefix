...
@section('title', 'Visit ' . store_name() . ' Showroom & Contact Desk')
@section('meta_description', 'Visit ' . store_name() . ' showroom in ' . store_city() . '. Get store directions, contact numbers, WhatsApp chat, and submit smartphone inquiries.')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <div id="location" class="lg:col-span-5 space-y-6">
                <div>
                    ...
                    <p class="apple-body text-apple-muted-48 mt-1">Conveniently located in {{ store_city() }}.</p>
                </div>
                ...
                <div class="apple-utility-card space-y-2">
                    ...
                    <h3 class="apple-body-strong text-apple-ink text-[17px]">{{ store_name() }}</h3>
                    <p class="apple-body text-apple-muted-80 leading-relaxed text-[15px]">
                        {{ store_address() }}
                    </p>
                    <div class="pt-2">
                        <a href="https://maps.google.com/?q={{ urlencode(store_address()) }}" target="_blank" class="apple-text-link text-[14px]">
                ...
                <div class="apple-utility-card space-y-3">
                    ...
                    <div class="space-y-2">
                        <a href="tel:{{ preg_replace('/[^0-9]/', '', store_phone()) }}" class="flex items-center justify-between p-3 rounded-[12px] bg-apple-parchment hover:bg-apple-hairline/60 transition-colors text-decoration-none">
                            ...
                            <span class="apple-body-strong text-apple-primary text-[15px]">{{ store_phone() }}</span>
                        </a>
                        ...
                        @if(!empty(store_landline()))
                        <a href="tel:{{ preg_replace('/[^0-9]/', '', store_landline()) }}" class="flex items-center justify-between p-3 rounded-[12px] bg-apple-parchment hover:bg-apple-hairline/60 transition-colors text-decoration-none">
                            <div class="flex items-center gap-3">
                                <i data-lucide="phone-call" class="w-4 h-4 text-apple-primary"></i>
                                <span class="apple-body text-apple-ink text-[15px]">Landline Support</span>
                            </div>
                            <span class="apple-body-strong text-apple-primary text-[15px]">{{ store_landline() }}</span>
                        </a>
                        @endif
                    </div>
                </div>
            ...
            <div id="inquiry" class="lg:col-span-7 apple-utility-card !p-8 sm:!p-12 space-y-6">
                ...
                <form action="{{ route('public.contact.submit') }}" method="POST" class="space-y-4">
                    ...
                    <p class="apple-fine-print text-apple-muted-48 text-center pt-2">
                        By submitting this form, you consent to receive a direct call or SMS regarding your request from our {{ store_name() }} store desk.
                    </p>
                </form>
            </div>
