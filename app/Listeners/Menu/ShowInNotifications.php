    ...
    public function handle(Event $event)
    {
        ...
        if (user()->can('read-install-updates')) {
            ...
            foreach ($updates as $key => $update) {
                ...
                $update_url = \Illuminate\Support\Facades\Route::has('updates.index') ? route('updates.index') : '#';
                $new->data = [
                    'title' => $name . ' (v' . $update?->latest . ')',
                    'description' => trans('install.update.' . $prefix, ['module' => $name, 'url' => $update_url]),
                ];
        ...
        foreach ($new_apps as $key => $new_app) {
            ...
            $app_url = \Illuminate\Support\Facades\Route::has('apps.app.show') ? route('apps.app.show', [
                'alias'         => $new_app->alias,
