...
class DashboardController extends BaseMobileShopController
{
    ...
    public function dashboard()
    {
        ...
        // ─── Admin-Only Executive Analytics (Cached 60s) ────────────────────
        $analytics = [];
        if ($isAdmin) {
            $analytics = Cache::remember("ms_dash_analytics_{$companyId}", 60, function () use ($companyId, $lowStockParts) {
                ...
                // 7-day trend (Aggregated in 3 fast queries instead of 21 loop queries)
                ...
                $cogsGrouped7 = DB::table('ms_mobile_sales')
                    ->join('ms_mobile_devices', 'ms_mobile_sales.device_id', '=', 'ms_mobile_devices.id')
                    ->where('ms_mobile_sales.company_id', $companyId)
                    ->where('ms_mobile_sales.status', '!=', 'voided')
                    ->where('ms_mobile_sales.created_at', '>=', $sevenDaysAgo)
                    ->selectRaw("DATE({$prefix}ms_mobile_sales.created_at) as d, SUM({$prefix}ms_mobile_devices.purchase_cost) as total")
                ...
                for ($d = 6; $d >= 0; $d--) {
                    ...
                    $daySales = (float) (($mobGrouped7[$dateStr] ?? 0) + ($accGrouped7[$dateStr] ?? 0));
                    $dayCogs  = (float) ($cogsGrouped7[$dateStr] ?? 0);
                    $sevenDayTrend[] = [
