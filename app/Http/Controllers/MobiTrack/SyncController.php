...
{
    /**
     * Allowed models and their whitelisted columns for synchronization.
     */
    ...
    public function pending(Request $request)
    {
        ...
        $items = MSSyncQueue::where('company_id', $companyId)
    ...
    public function upload(Request $request)
    {
        ...
        try {
            foreach ($request->operations as $op) {
                try {
                    $results[] = $this->applyOperation($op, $companyId);
                } catch (\Throwable $e) {
    ...
    public function offset(Request $request)
    {
        ...
        foreach ($request->offsets as $item) {
            if (!array_key_exists($item['model'], $this->whitelist)) {
                continue;
            }

            MSSyncOffset::updateOrInsert(
                ['model' => $item['model'], 'company_id' => $companyId],
    ...
    public function status(Request $request)
    {
        ...
        return response()->json([
            'success' => true,
            'data' => [
                'pending_count' => MSSyncQueue::where('company_id', $companyId)->where('status', 'pending')->count(),
                'failed_count' => MSSyncQueue::where('company_id', $companyId)->where('status', 'failed')->count(),
                'last_sync_at' => MSSyncOffset::where('company_id', $companyId)->where('model', 'ms_mobile_sales')->value('last_synced_at'),
            ],
        ]);
    }

    protected function applyOperation(array $op, int $companyId): array
    {
        // 1. Strict model whitelist validation
        if (!array_key_exists($op['model'], $this->whitelist)) {
            throw new \InvalidArgumentException("Unauthorized table/model: {$op['model']}");
        }
        ...
        // 2. Fail-Closed Multi-Tenancy: force tenant company_id into payload
        ...
        switch ($op['action']) {
            ...
            case 'update':
                ...
                // Verify record exists and belongs to this tenant company
                $existing = DB::table($op['model'])
                    ->where('id', $op['record_id'])
                    ->where('company_id', $companyId)
                    ->first();

                if (!$existing) {
                    throw new \InvalidArgumentException("Record #{$op['record_id']} not found in {$op['model']} for company {$companyId}.");
                }
        ...
        if (!empty($op['sync_key'])) {
            MSSyncQueue::updateOrInsert(
                ['sync_key' => $op['sync_key'], 'company_id' => $companyId],
                [
                    'action' => $op['action'],
                    'model' => $op['model'],
                    'status' => 'uploaded',
        ...
        MSSyncOffset::updateOrInsert(
            ['model' => $op['model'], 'company_id' => $companyId],
            [
                'last_synced_id' => max(
                    $serverId ?? 0,
                    MSSyncOffset::where('model', $op['model'])->where('company_id', $companyId)->value('last_synced_id') ?? 0
                ),
                'last_synced_at' => now()
            ]
        );
